...
namespace GBC_Travel_Group_77.Areas.Identity.Pages.Account
{
    public class ConfirmEmailModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string userId, string code)
        {
            try
            {
                _logger.LogInformation("Calling the OnGetAsync action in ConfirmEmailModel");
                if (userId == null || code == null)
                {
                    _logger.LogInformation("The user id is null or the code is null");
                ...
                if (user == null)
                {
                    _logger.LogInformation("The user variable is null");
